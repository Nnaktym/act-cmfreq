"""Multi-seed robustness check for the VehModel x AREA theft テスト2 paper.

Same as vehmodel_state_theft_multiseed.py but col_col="Area". Writes
docs/validation_multiseed_vehmodel_area_theft.csv.
"""
import os
...
def main():
    ...
    for seed in SEEDS:
        ...
        best = optimize_params(X_train, n_folds=4, k_values=K_GRID,
                               lambda_values=LAMBDA_GRID, W=W_full, random_seed=seed)
        ...
        rows.append({"seed": seed, "k": best["k"], "lambda": best["lambda"],
                     "n_eval": int(eval_mask.sum()), "MF_wRMSE": mf_w,
                     "GLM_wRMSE": glm_w, "MF_minus_GLM": mf_w - glm_w})
        print(rows[-1])

    df = pd.DataFrame(rows)
    df.to_csv(f"{DOCS_DIR}/validation_multiseed_vehmodel_area_theft.csv", index=False)
    print("\n==== multi-seed summary ====")
    print(df.to_string(index=False))
    print(f"\nMF wins {int((df['MF_minus_GLM'] < 0).sum())}/{len(df)} seeds; "
          f"mean MF-GLM = {df['MF_minus_GLM'].mean():.1f}, sd = {df['MF_minus_GLM'].std():.1f}")
