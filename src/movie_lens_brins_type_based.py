...
brazil_data_path = "/Users/spectee/research/act-cmfreq/data/brvehins_org.csv"
df = pd.read_csv(brazil_data_path)
# df = df[df['Gender'] != 'Corporate']
df.columns


df = df[['VehModel', 'Area', 'ExposTotal', 'ClaimTotal']]
df = df.groupby(['VehModel', 'Area']).sum().reset_index()

# should change this to all categories

# df["ClaimTotal"] = np.where(df["ClaimTotal"] > 0, df["ClaimTotal"], np.nan)
threshold = 10
df["ExposTotal"] = np.where(df["ExposTotal"] > threshold , df["ExposTotal"], np.nan)
threshold = 0
df["ClaimTotal"] = np.where(df["ClaimTotal"] > threshold , df["ClaimTotal"], np.nan)
# df.to_csv("t.csv")

df["pure_premium"] = df["ClaimTotal"] / df["ExposTotal"]
# df = df.merge(density, on='Area', how='left')
# df = df.reset_index(drop=True)

df = df[['VehModel', 'Area', 'pure_premium']]

all_vehicle_models = df["VehModel"].unique()
all_areas = df["Area"].unique()

formatted_df = pd.DataFrame()
formatted_df["VehModel"] = np.repeat(all_vehicle_models, len(all_areas))
formatted_df["Area"] = np.tile(all_areas, len(all_vehicle_models))

formatted_df = formatted_df.merge(df, on=["VehModel", "Area"], how="left")
...
policy_side_info_cols = ["VehModel"] # 'VehModel'
area_side_info_cols = ['Area'] # , 'density_km2']
ratings_cols = ["pure_premium"]

ratings = df[cols_to_use]
# ratings = ratings[ratings["pure_premium"] > 0]

policy_side_info = df[policy_side_info_cols]
area_side_info = df[area_side_info_cols]
area_id_to_name = {i: j for i, j in zip(density["Area"], density["Area"])}

ratings = ratings.rename(columns={"VehModel": "UserId", "Area": "ItemId", "pure_premium": "Rating"})
policy_side_info = policy_side_info.rename(columns={"VehModel": "UserId"})
area_side_info = area_side_info.rename(columns={"Area": "ItemId"})

# train test split ratings
train, test = train_test_split(ratings, test_size=0.2, random_state=42)
# train, test = train_test_split(ratings.dropna(), test_size=0.2, random_state=42)
# may need to adjust number of rows in train and test by removing rows with NaN values
...
# train = pd.read_csv("data/split$X_train.csv", index_col=0)
# train = train.reset_index()
# train = train.melt(id_vars=["index"], var_name="ItemId", value_name="Rating")
# train = train.rename(columns={"index": "UserId"})
# train = train.dropna()

# test = pd.read_csv("data/split$X_test.csv", index_col=0)
# test = test.reset_index()
# test = test.melt(id_vars=["index"], var_name="ItemId", value_name="Rating")
# test = test.rename(columns={"index": "UserId"})
# test = test.dropna()
...
model_no_sideinfo = CMF(
    ...
    method='als',
    use_cg=False,
    user_bias=False, #True,
    item_bias=False, #True,
    center=False,
...
pred = model_no_sideinfo.predict(user=test.dropna()["UserId"], item=test.dropna()["ItemId"])
rmse = np.sqrt(mean_squared_error(pred, test.dropna()["Rating"]))

plot_actual_vs_predicted(test.dropna(), pred, "classical_model")


train[train["UserId"] == "Honda - Accord Coupe Ex"]
