...
premium_total = tapply(brvehins$PremTotal, list(brvehins$VehModel,brvehins$Area,brvehins$cvgroup), sum) # ?^?E?n??ECV?O???[?v??????????W?v
claim_total = tapply(rowSums(brvehins[,19:23]), list(brvehins$VehModel,brvehins$Area,brvehins$cvgroup), sum) # ?^?E?n??ECV?O???[?v????N???[?????z??W?v
print(dim(premium_total))
print(dim(claim_total))


# ??????v10000???A?N???[?????z?v5000????^?????c??
...
loss_ratio_cvtrain = claim_total_cvtrain / premium_total_cvtrain # ?^?E?n?斈????Q??
...
# ?????????j=21?ilambda=2^21?j?A???????????j=22?ilambda=2^22?j??????????????
#> rowMeans(cverror_nonneg)
...
# A??B??????lambda???????????0????????????ii.e.A=B=0?????s???????????minimum-bias?@????????j
...
cmf = CMF( # <----- ???????????????F?X??????
  ...
  nonneg=TRUE, # added
  weight=premium_total_cvtrain,
  lambda=c(0,0,1e20,1e20,0,0), # ????'user_bias', 'item_bias', 'A', 'B', 'C', 'D'??????lambda?i'C', 'D'??{?????g?p???????j
)
A = cmf$matrices$A
B = cmf$matrices$B
type_bias = cmf$matrices$user_bias
...
# ???Q?????`
loss_ratio = apply(claim_total_10000,1:2,sum) / apply(premium_total_10000,1:2,sum) # ?^?E?n?斈????Q??
loss_ratio[apply(premium_total_10000,1:2,sum)==0] = NA # added?iloss_ratio??Inf???????cmf$matrices$glob_mean??NA?????K???????????????????????????j
...
# ?_??????Met. de Sao Paulo?ARibeirao Preto e Demais Mun. de Campinas??????p??????
> result_area
