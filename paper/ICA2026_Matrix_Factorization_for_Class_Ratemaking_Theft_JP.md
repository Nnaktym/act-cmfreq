...
　観測行列には、**両方**のリスク因子が効くという解釈可能な構造が現れている（以下すべて露出加重。全体の露出加重盗難純保険料率は234、セル単位の分布は強く右に歪みかつゼロ過剰で、中央値95・90パーセンタイル436、観測セルの約23%は盗難クレームがゼロ）。露出加重の盗難純率は、車種グループ間で0〜約1,810、州間で約2〜329に及び、*両軸とも*広い開きを持つ。車種グループのうち盗難コストが最も高いのは大型トラック（Scania、Iveco Stralis、Volvo の *Caminhões*）、商用バン（Mercedes-Benz Sprinter）、大型二輪（>450cc の Yamaha・Suzuki）、および高級車（Peugeot 407、Toyota Land Cruiser）であり、最も低いのは小型二輪と旧型大衆車である。これは修理コストではなく、車両の価値と盗難者にとっての魅力を反映した順序である。際立つのは地理的勾配である。盗難コストは人口密度の高い南東部・沿岸大都市—Rio de Janeiro（329）と São Paulo（314）、次いで Bahia、Paraná、Rio Grande do Sul—で最も高く、人口の希薄な北部・フロンティア州（Rondônia ≈ 2、Amapá ≈ 2、Roraima ≈ 7）で最も低い。両軸がともに強い信号を持つため、露出加重の二元配置（車種グループ＋州）主効果モデルが説明できるのはセル盗難純保険料率の露出加重変動の約**67%**にとどまり、車種グループ×州の交互作用とセル水準のノイズに約**33%**が残される。この大きな交互作用の余地が、第4.5節で交差検証が選ぶ高ランクの分解（k=27）を予告している。

　図4.2.2は、2組の周辺効果を同一の純保険料率軸上に描く。車種グループと州の両者がともに広く広がっており、これは上述の「両軸が効く」という読み、および続く大きな交互作用の余地の視覚的な対応物である。

![fig_4_2_2](fig_4_2_2_theft.png)

図4.2.2：車種グループ（左）および州（右）別のソート済み周辺効果。盗難では両軸がともに実質的な信号を持つ。
...
潜在因子数 k と正則化の重み λ は、グリッド上での4分割交差検証によって選択し、平均の露出加重RMSE（Root Mean Square Error）を最小化する組合せを選んだ。テストセットはこのチューニングの前に取り分けてあり、ハイパーパラメータ選択が評価セルを見ることはない。盗難の行列分解では、この手続きは**高ランク（k=27。探索グリッドの上限）と正則化の重み λ=100** を選んだ。ここでは交差検証が追加の潜在次元を実際に報いており、第4.2節で示した大きな交互作用の余地（主効果2つで捉えられる露出加重変動は約67%にすぎない）と整合的である。言い換えれば、盗難は車種×地域の交互作用が十分に大きく、モデルがそれを表現するために豊かな低ランク構造を欲するペリルであり、まさに行列分解が想定する領域である。
...
本研究では、**車両盗難ペリル**のクラス別料率算定に焦点を当て、多数のカテゴリ、著しいスパース性、そして実質的な車種×地域の交互作用を特徴とする目的変数に対するアプローチとして行列分解（MF）を検討した。本手法を実世界の保険データに適用した結果は以下のとおりである。盗難の交互作用構造は実在し、かつ大きい。二元配置の主効果モデルは露出加重変動の約3分の1を説明できず、ベイジアンGLMMは大きな交互作用標準偏差を識別し（σ ≈ 0.143）、交差検証は高ランクの分解を選ぶ（k=27）。予測精度の像は混在しており、誇張せず報告する。行列分解は非加重RMSEで最も精度が高く（237.27 対 GLMの253.78）、**スパース**な露出層では露出を考慮した両指標でGLMを上回る（露出加重RMSE 244.51 対 270.02、ポアソン逸脱度 $2.20\times10^7$ 対 $3.59\times10^7$）。しかし全セルでプールした露出加重指標では主効果GLMが上回る（178.52 対 195.20）。これはそれらの平均がデータ豊富な密セルに支配され、そこでは主効果で十分だからである。GLMMは、各ホールドアウトセルが未観測交互作用でありその変量効果がゼロに回帰するためGLMと同点となる。したがってMFの決定的な優位は、プールした精度の勝敗ではなく構造にある。すなわち、標本内では大きな交互作用を当てはめながら未観測の組合せへは持ち出せないGLMMとは異なり、MFは観測露出のないものを含め、すべての車種グループ×州セルに対して交互作用を考慮した推定を生成する。盗難は、採択済みアブストラクトの「GLMがしばしば失敗するスパースなセグメント」を特に明瞭に示す事例である。交互作用は明白に存在し、GLMMはそれを見ることはできても外挿できず、MFは3手法の中で唯一、スパース・欠測セグメントにわたって交互作用を考慮した料率を供給し、しかもまさにそこで最も精度の高いモデルであり続ける。我々はMFを、確立されたアクチュアリーモデルの全面的な代替としてではなく、まさにこのスパースで高交互作用の設定でこそ最大の価値を持つ補完的ツールとして位置づける。
