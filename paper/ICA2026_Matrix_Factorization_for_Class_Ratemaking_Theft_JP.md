...
　本研究の目的はクラス料率算定への応用であるため、観測行列 X は事故コスト、具体的には盗難（robbery）の過去の純保険料率（historical pure premium rate）から構成される。レコメンダーシステムでは観測行列 X の行と列は典型的にユーザーとアイテムを表すが、本研究の料率算定への応用では、多数のカテゴリを持つ2つのリスク因子を行と列に対応させる。多数のカテゴリを持つリスク因子としては、次節の応用例で論じるとおり、地理的地域や車種といった変数を想定する。
...
　盗難を分析対象に選んだのは、そのリスクが車種と地域の**交互作用**として現れることが先験的に強く予想され、交互作用を低ランク構造として推定する行列分解の実務的価値を示す好個の題材だからである。盗難は修理費のように車両単独で決まる（車種の主効果に近い）のではなく、車両の価値や転売市場にとっての魅力（誘因）と、地域の都市化・盗難車の転売／解体市場・検挙水準といった治安環境（機会）とが**重なる**セルで突出すると考えられ、これは主効果の和では捉えきれない交互作用そのものである。加えてこの交互作用は、車種の主効果が支配的な衝突（collision）とは地理的な向きが異なることも予想される。
...
　本研究では、特に多数のカテゴリを持つリスク因子である車種グループと州に焦点を当てる。予測の目的変数は、**盗難（robbery）の過去の純保険料率**（事故コスト＝盗難クレーム総額 `ClaimAmountRob` ÷ 総露出）である。ここでデータ上の注意点を述べておく。brvehins1 には火災・盗難担保専用の露出列 `ExposFireRob` が定義されているが、この列（および対になる保険料 `PremFireRob`）は**全データシャードでゼロのまま出荷されている**（生の `.rda` シャードを直接読み込んで確認した）。したがって盗難専用の露出基盤は存在しない。そこで本分析では全体露出 `ExposTotal` を分母に用いる。これは本 AUTOSEG 抽出において全ペリル（盗難・衝突・火災・その他）のクレームが対して発生する包括担保の露出であり、セル間で比較可能な共通の分母である（ただし盗難専用のものではない）。

　モデルの学習には全製造業者のデータを用いる。分析にあたっては、まず4,259の車種（VehModel）を436の車種グループ（VehGroup、上位のモデル系列）へと集約し、より粗く密な粒度で行列を構成する。契約量が少ない場合には過去データから純保険料率を統計的に決定するのが難しいため、総露出が100以上のセル（車種グループ×州の組合せ）を観測とみなし（それ未満は欠測として補完対象とする）、かつ総露出が10を超える車種グループのみをモデル構築に用いる。この結果、学習用の行列は231車種グループ × 27州（ブラジルの27連邦単位）、観測セル数2,233（全体の約36%が観測）となる。その他すべての組合せは、提案する行列分解アプローチによって推定される欠測値として扱う。この231車種グループ × 27州の行列は十分に小さいため全体をそのまま可視化でき、本節および以降のヒートマップ（図4.2.1、4.3.1、4.3.2、4.4.1、4.4.2、4.5.2）は、フィルタ後に残った231の車種グループ（行）すべてを27州（列）にわたって表示する。第4.6節の数値比較も同一の行列に基づく。ただし231行すべてに読めるラベルを付すことはできないため、縦軸のラベルは省略する。その代わり、すべてのヒートマップにおいて行・列の両軸を露出加重の周辺純保険料率（低→高）で並べ替えており、支配的なコスト勾配が左上から右下へ滑らかに走り、車種グループ×州の交互作用はその並び順からの逸脱として読み取れる。全ヒートマップは共通のカラースケールを用い、右に歪んだ純保険料率の分布を暗部に潰さず広げるため対数スケールで描画する。また低露出のため欠測として扱うセルはグレーで示す。図4.2.1は、27州にわたる実際の事故コスト（過去の純保険料率）を示している。この欠測は完全にランダム（MCAR）ではない。すなわち、あるセルが未観測であるのはまさにそのセルの露出がほとんど、あるいは全くないためであり、そのこと自体がそのセグメントに関する情報を持つ。したがっていかなる補完も、観測セルから学習された潜在構造がこれら系統的に異なるセルにも及ぶという仮定に依拠しており、この仮定は欠測セルの真値（ground truth）が存在しない以上、直接検証することはできない。

![fig_4_2_1](fig_4_2_1_theft.png)

図4.2.1：車種グループおよび州別の実際の事故コスト

　観測行列には、**両方**のリスク因子が効くという解釈可能な構造が現れている—これは車種グループが支配的だった衝突との決定的な違いである（以下すべて露出加重。全体の露出加重盗難純保険料率は234、セル単位の分布は強く右に歪みかつゼロ過剰で、中央値95・90パーセンタイル436、観測セルの約23%は盗難クレームがゼロ）。露出加重の盗難純率は、車種グループ間で0〜約1,810、州間で約2〜329に及び、*両軸とも*広い開きを持つ。車種グループのうち盗難コストが最も高いのは大型トラック（Scania、Iveco Stralis、Volvo の *Caminhões*）、商用バン（Mercedes-Benz Sprinter）、大型二輪（>450cc の Yamaha・Suzuki）、および高級車（Peugeot 407、Toyota Land Cruiser）であり、最も低いのは小型二輪と旧型大衆車である。これは修理コストではなく、車両の価値と盗難者にとっての魅力を反映した順序である。際立つのは地理的勾配で、衝突とは**逆向き**である。盗難コストは人口密度の高い南東部・沿岸大都市—Rio de Janeiro（329）と São Paulo（314）、次いで Bahia、Paraná、Rio Grande do Sul—で最も高く、人口の希薄な北部・フロンティア州（Rondônia ≈ 2、Amapá ≈ 2、Roraima ≈ 7）で最も低い。後者はまさに衝突コストが*最も高かった*州である。27州を両ペリルで順位づけるとスピアマン相関は −0.64 となる一方、車種グループの順位はおおむね共通する（+0.62）。両軸がともに、しかも逆向きの強い信号を持つため、露出加重の二元配置（車種グループ＋州）主効果モデルが説明できるのはセル盗難純保険料率の露出加重変動の約**67%**にとどまり、車種グループ×州の交互作用とセル水準のノイズに約**33%**が残される—衝突での約17%のほぼ2倍である。この大きな交互作用の余地が、第4.5節で交差検証が選ぶ高ランクの分解（k=27。衝突では k=2）を予告している。

　図4.2.2は、2組の周辺効果を同一の純保険料率軸上に描く。車種グループの曲線が州の棒を圧倒していた衝突とは対照的に、ここでは両者がともに広く広がる。これは上述の「両軸が効く」という読み、および続く大きな交互作用の余地の視覚的な対応物である。

![fig_4_2_2](fig_4_2_2_theft.png)

図4.2.2：車種グループ（左）および州（右）別のソート済み周辺効果。盗難では両軸がともに、しかも逆向きの、実質的な信号を持つ。
...
　車種グループおよび州別の純保険料率の推定結果を図4.3.1と図4.3.2に示す。図4.3.2では、前節と同様に、グレーの領域は推定に十分なデータがなかった欠測値を表す。
...
グレーの領域は推定に十分なデータがなかった**欠測値**を表す（交互作用項なしのGLM）
...
- 欠測値への外挿：全カテゴリに一律の係数を適用することで、欠測データのセルについても料率を計算できる。しかし、モデル構築用データセットに全く含まれていなかったカテゴリ（ヒートマップ上のグレーの領域）については、その結果は観測データに基づく推定ではなく単なる外挿にすぎない。
...
図4.4.2：車種グループおよび州別の推定純保険料率。グレーの領域は推定に十分なデータがなかった欠測値を表す（変量効果を持つGLMM）
...
潜在因子数 k と正則化の重み λ は、グリッド上での4分割交差検証によって選択し、平均の露出加重RMSE（Root Mean Square Error）を最小化する組合せを選んだ。テストセットはこのチューニングの前に取り分けてあり、ハイパーパラメータ選択が評価セルを見ることはない。盗難の行列分解では、この手続きは**高ランク（k=27。探索グリッドの上限）と正則化の重み λ=100** を選んだ。これは k=2 が選ばれた衝突分析との顕著な対比である。ここでは交差検証が追加の潜在次元を実際に報いており、第4.2節で示した大きな交互作用の余地（主効果2つで捉えられる露出加重変動は約67%にすぎない）と整合的である。言い換えれば、盗難は車種×地域の交互作用が十分に大きく、モデルがそれを表現するために豊かな低ランク構造を欲するペリルであり、まさに行列分解が想定する領域である。
...
![fig_4_6_1](fig_4_6_1_theft.png)

図4.6.1：純保険料率の予測値対真値（集合行列分解、CMF）

推定結果の特徴を以下にまとめる：

- **盗難でも全体としてサイド情報は精度を改善しない：** 後掲の表4.5.1が示すとおり、CMFはプールしたRMSEと露出加重RMSEで素の行列分解を下回る（$237.27 \to 263.01$、$195.20 \to 205.53$）。メーカーと州の人口密度区分は、潜在因子がすでに捉える以上の信号をほとんど持たなかった。交差検証がサイド情報の重みを探索範囲の低い側 $(1.0,\ 0.15,\ 0.15)$ に寄せて選んだことも、これと整合的である。

- **ただしスパース層では示唆的な例外がある：** スパースな露出層に限れば、CMFは全モデル中で最も低いポアソン逸脱度（$2.03\times10^7$。MFの $2.20\times10^7$、GLMの $3.59\times10^7$ を下回る）を達成した。これは、サイド情報がまさにデータの最も薄い箇所で最も効きうることを示唆する。

- **CMFの価値は観測セルの精度ではなく構造にある：** 観測セル全体の再現では素の行列分解が最も精度が高く、CMFはそれに及ばない（表4.5.1）。CMFの価値は、行列分解と同様、従来モデルが交互作用構造を提供できないスパース・欠測セグメントに対して、観測可能な属性に基づく推定を供給できる点にある。
...
表4.5.1：観測セルの同一547セット上でのホールドアウト予測性能（低いほど良い）。ハイパーパラメータは、最終フィットと同じ非負・非中心化（non-centered）・露出加重の仕様を用いた交差検証で再選択した。盗難では基準が豊かな分解を報いる（行列分解は $k=27$, $\lambda=100$、CMFは $k=3$, $\lambda=1000$、サイド情報の重み $(1.0, 0.15, 0.15)$）。
...
| 集合行列分解（CMF）‡ | 263.01 | 205.53 | 1.61×10⁸ |
...
プールした全体像は率直に言って混在しており、そのまま報告する。行列分解は**非加重RMSE**で最小（237.27 対 GLMの253.78）であり、高ランクで捉えた交互作用構造を反映している。しかし2つの**露出加重**指標では、547セル全体でプールすると主効果GLMが上回る（露出加重RMSE 178.52 対 195.20、ポアソン逸脱度 $1.45\times10^8$ 対 $2.04\times10^8$）。GLMMは（各ホールドアウトセルが未観測交互作用で変量効果がゼロに回帰するため）GLMと完全に同点となる。これは衝突で見られた結果とは逆であり、その理由は示唆に富む。露出加重指標は最も露出の大きい（密な）セルに支配され、層別表が示すとおり、その密な領域こそGLMの主効果が最も打ち破りにくい場所だからである。スパースなペリルにとって重要な問いは、露出加重のプール平均ではなく、データが薄い箇所での性能である。

利用可能な唯一の真値—低露出の観測セル—を用いてスパース領域を精査するため、ホールドアウトセルを露出の中央値で層別した（表4.5.2）。ここでは結果が反転し、分解に有利となる。**スパース**層では、行列分解は露出を考慮した両指標でGLMを上回る（露出加重RMSE 244.51 対 270.02、ポアソン逸脱度 $2.20\times10^7$ 対 $3.59\times10^7$）。さらにサイド情報付きのCMFは逸脱度でこれを下回る（$2.03\times10^7$）。**密**層では順位が逆転しGLMが最良となる（露出加重RMSE 171.71 対 191.95、逸脱度 $1.09\times10^8$ 対 $1.82\times10^8$）。すなわち行列分解の優位は、本稿が関心を寄せる低露出セグメントにまさに集中しており、GLMのプール勝ちは主効果で十分なデータ豊富なセルによって駆動されている。なおスパース層もあくまで代理指標である点を強調しておく。真に欠測したセルはさらに少ない（典型的にはゼロの）露出しか持たず、これらの数値のいずれによっても保証できる範囲の外にある。
...
| スパース（露出 < 中央値） | 集合行列分解（CMF） | 256.54 | **2.03×10⁷** |
| 密（露出 ≥ 中央値） | GLM / GLMM† | **171.71** | **1.09×10⁸** |
| 密（露出 ≥ 中央値） | 行列分解 | 191.95 | 1.82×10⁸ |
| 密（露出 ≥ 中央値） | 集合行列分解（CMF） | 202.17 | 1.40×10⁸ |
...
**限界。** 盗難の結果には2つの留意点がある。第一に、*属性構成の交絡*。セル目的変数（生の盗難純保険料率 = Σクレーム/Σ露出）は、車種グループ×州の交互作用に加えて各セルの契約者属性構成（性別・運転者年齢・車両年式のミックス）を混在させており、これはセル間で異なる。衝突分析で行ったレコード単位の属性標準化はここでは再実施していないため、観測セルの精度数値は、車種×地域の純粋な交互作用ではなく潜在因子が属性構成を部分的に吸収した結果を含みうるものとして読むべきである。第二に、*露出基盤*。第4.2節で述べたとおり、brvehins1 には盗難専用の露出が存在せず（`ExposFireRob` が空）、`ExposTotal` を代用の分母としている。仮に盗難担保の付保対象部分ポートフォリオが包括担保のそれと系統的に異なるなら、水準は（ただし必ずしもセル間の順位ではなく）影響を受けうる。いずれの留意点も、MFの主たる主張—従来モデルが交互作用構造を提供できないスパース・欠測セグメントへ交互作用を考慮した推定を拡張するという構造的な利点—を損なうものではない。
...
本研究では、**車両盗難ペリル**のクラス別料率算定に焦点を当て、多数のカテゴリ、著しいスパース性、そして—衝突とは異なり—実質的な車種×地域の交互作用を特徴とする目的変数に対するアプローチとして行列分解（MF）を検討した。本手法を実世界の保険データに適用した結果は以下のとおりである。盗難の交互作用構造は実在し、かつ大きい。二元配置の主効果モデルは露出加重変動の約3分の1を説明できず（衝突では約6分の1）、ベイジアンGLMMは約4倍大きい交互作用標準偏差を識別し（σ ≈ 0.143 対 0.033）、交差検証は高ランクの分解を選ぶ（k=27 対 k=2）。予測精度の像は混在しており、誇張せず報告する。行列分解は非加重RMSEで最も精度が高く（237.27 対 GLMの253.78）、**スパース**な露出層では露出を考慮した両指標でGLMを上回る（露出加重RMSE 244.51 対 270.02、ポアソン逸脱度 $2.20\times10^7$ 対 $3.59\times10^7$）。しかし全セルでプールした露出加重指標では主効果GLMが上回る（178.52 対 195.20）。これはそれらの平均がデータ豊富な密セルに支配され、そこでは主効果で十分だからである。GLMMは、各ホールドアウトセルが未観測交互作用でありその変量効果がゼロに回帰するためGLMと同点となる。したがってMFの決定的な優位は、プールした精度の勝敗ではなく構造にある。すなわち、標本内では大きな交互作用を当てはめながら未観測の組合せへは持ち出せないGLMMとは異なり、MFは観測露出のないものを含め、すべての車種グループ×州セルに対して交互作用を考慮した推定を生成する。盗難は、採択済みアブストラクトの「GLMがしばしば失敗するスパースなセグメント」を特に明瞭に示す事例である。交互作用は明白に存在し、GLMMはそれを見ることはできても外挿できず、MFは3手法の中で唯一、スパース・欠測セグメントにわたって交互作用を考慮した料率を供給し、しかもまさにそこで最も精度の高いモデルであり続ける。我々はMFを、確立されたアクチュアリーモデルの全面的な代替としてではなく、まさにこのスパースで高交互作用の設定でこそ最大の価値を持つ補完的ツールとして位置づける。

　本研究は提案アプローチの有効性を確認したが、さらなる発展の余地がいくつか残されている：

- サイド情報の統合：本稿では第4.6節で、主要な事故行列をメーカーと州の人口密度区分の補助属性行列とともに同時に分解する集合行列分解（Collective Matrix Factorization, CMF）を検証した。盗難を目的変数とした本データセットでは、サイド情報は全体としての精度を改善せず、CMFはプールしたRMSE・露出加重RMSEで素の行列分解を下回った（237.27→263.01、195.20→205.53）。交差検証がサイド情報の重みを探索範囲の低い側に寄せて選んだことも、これと整合的である。ただし示唆的な例外として、スパースな露出層ではCMFが全モデル中で最も低いポアソン逸脱度（$2.03\times10^7$）を達成しており、サイド情報がまさにデータの最も薄い箇所で最も効きうることを示唆する。より豊富な属性（車両の物理特性、より細かい地理的共変量など）がこれを一貫した利得に変えうるかは、今後の課題である。

- 純保険料に整合した損失族：提案モデルは（加重）二乗誤差損失を最小化するが、これは非負で裾が重く、盗難では強くゼロ過剰な純保険料を近似的にガウス分布として扱い、最大のセルに支配される。この不整合は、盗難で行列分解が露出加重ポアソン逸脱度において劣った（それ自身が最適化していない基準で採点された）最も有力な説明であり、観測盗難セルの約4分の1がゼロを記録するため、衝突よりも深刻である。したがって目的関数を事故生成過程に整合させることは、このペリルにとって特に自然な改良である。TweedieまたはポアソンガンマのDeviance損失（例：Jørgensen & Paes de Souza, 1994; Ohlsson & Johansson, 2010）の下で分解を当てはめれば、盗難クレームの頻度・severity構造に目的関数を整合させることができ、サイド情報付き変種を強化する可能性もある。
