...
　本研究の目的はクラス料率算定への応用であるため、観測行列 X は事故コスト、具体的には衝突事故（部分衝突＋全損衝突）の過去の純保険料率（historical pure premium rate）から構成される。レコメンダーシステムでは観測行列 X の行と列は典型的にユーザーとアイテムを表すが、本研究の料率算定への応用では、多数のカテゴリを持つ2つのリスク因子を行と列に対応させる。多数のカテゴリを持つリスク因子としては、次節の応用例で論じるとおり、地理的地域や車種といった変数を想定する。
...
本節では、自動車保険の事故データを用い、特に行列分解技術を用いて車種グループおよび州別の純保険料率を推定する。評価のベースラインを提供するため、まず2つの従来手法—交互作用項を持たない一般化線形モデル（GLM）と、交互作用を変量効果として扱う一般化線形混合モデル（GLMM）—の推定結果を提示する。続いて、比較対象として提案する行列分解アプローチによる推定結果を示す。これらの結果に基づき、保険業界におけるクラス別料率の決定に行列分解を適用することの実務的な有効性と利点を論じる。
...
　本研究では、特に多数のカテゴリを持つリスク因子である車種グループと州に焦点を当てる。予測の目的変数は、衝突事故（部分衝突＋全損衝突）の過去の純保険料率（事故コスト＝衝突事故の総事故金額÷総露出）である。全ペリルのうち衝突事故に限定するのは、盗難・全損・火災といった低頻度・高severityの裾ペリルがL2損失を支配して交互作用構造の推定を歪める一方、件数の大半を占める「その他」ペリルは平均severityが極めて小さく信号として希薄なためである。衝突事故（部分衝突＋全損衝突）は全事故金額の約6割・件数の約3割を占め、料率算定の信号として十分な量を持ち、かつ裾ペリルによる支配が緩和される。

　モデルの学習には全製造業者のデータを用いる。分析にあたっては、まず4,259の車種（VehModel）を436の車種グループ（VehGroup、上位のモデル系列）へと集約し、より粗く密な粒度で行列を構成する。契約量が少ない場合には過去データから純保険料率を統計的に決定するのが難しいため、総露出が100以上のセル（車種グループ×州の組合せ）を観測とみなし（それ未満は欠測として補完対象とする）、かつ総露出が10を超える車種グループのみをモデル構築に用いる。この結果、学習用の行列は231車種グループ × 27州（ブラジルの27連邦単位）、観測セル数2,233（全体の約36%が観測）となる。その他すべての組合せは、提案する行列分解アプローチによって推定される欠測値として扱う。この231車種グループ × 27州の行列は十分に小さいため全体をそのまま可視化でき、本節および以降のヒートマップ（図4.2.1、4.3.1、4.3.2、4.4.1、4.4.2、4.5.2）は、フィルタ後に残った231の車種グループ（行）すべてを27州（列）にわたって表示する。第4.6節の数値比較も同一の行列に基づく。ただし231行すべてに読めるラベルを付すことはできないため、縦軸のラベルは省略し、車種グループ×州のパターンは色そのものによって示す。図4.2.1は、27州にわたる実際の事故コスト（過去の純保険料率）を示している。この欠測は完全にランダム（MCAR）ではない。すなわち、あるセルが未観測であるのはまさにそのセルの露出がほとんど、あるいは全くないためであり、そのこと自体がそのセグメントに関する情報を持つ。したがっていかなる補完も、観測セルから学習された潜在構造がこれら系統的に異なるセルにも及ぶという仮定に依拠しており、この仮定は欠測セルの真値（ground truth）が存在しない以上、直接検証することはできない。

![fig_4_2_1](fig_4_2_1.png)

図4.2.1：車種グループおよび州別の実際の事故コスト

　観測行列にはすでに解釈可能な構造が現れている（以下すべて露出加重。全体の露出加重衝突純保険料率は449、セル単位の分布は右に歪んでおり中央値500・90パーセンタイル1,243）。変動の主軸は州ではなく車種グループである。露出加重純率は車種グループ間でおよそ23〜4,020（約180倍の開き）に及ぶのに対し、州間では363〜1,179（約3.3倍）にとどまる。車種グループのうち衝突コストが最も高いのは大型商用トラック（Iveco Stralis、Volvo・Scania・Mercedes-Benz の *Caminhões* 等）と高級乗用車（BMW、Audi A4）、および大型ピックアップ（Toyota Hilux）であり、最も低いのは小型二輪（≤450cc の Honda・Suzuki・Yamaha）と旧型の大衆車（VW Fusca、Ford Pampa）である。この順序は車両の修理・再調達コストを反映しており、衝突コストを対象とする以上、想定どおりの結果である。州の勾配は人口密度と逆向きであり、人口の希薄な北部・フロンティア州（Acre、Amapá、Tocantins、Mato Grosso、Piauí、Pará）でコストが最も高く、南東部の成熟した高密度市場—São Paulo（露出は圧倒的に最大でありながら最も低く363）、Rio de Janeiro、Rio Grande do Sul—で最も低い。これはディーラー・部品網の充実が修理の重大度を下げるという解釈と整合する。露出加重の二元配置（車種グループ＋州）主効果モデルだけで、セル純保険料率の露出加重変動の約83%が説明され、車種グループ×州の交互作用とセル水準のノイズに残されるのは約17%にすぎない。これは、続く各節でGLMM（σ ≈ 0.033）と低ランクの行列分解（k=2）が捉える控えめな交互作用シグナルの、モデルによらない非公式な対応物である。
...
　車種グループおよび州別の純保険料率の推定結果を図4.3.1と図4.3.2に示す。図4.3.2では、前節と同様に、着色されていない（白い）領域は推定に十分なデータがなかった欠測値を表す。
...
図4.4.1：車種グループおよび州別の推定純保険料率（変量効果を持つGLMM）

（注：欠測値を含む全カテゴリへの外挿結果は、モデルが未観測の組合せに対して変量効果を一意に決定できないため省略している。）

![fig_4_4_2](fig_4_4_2.png)

図4.4.2：車種グループおよび州別の推定純保険料率。着色されていない（白い）領域は推定に十分なデータがなかった欠測値を表す（変量効果を持つGLMM）
...
潜在因子数 k と正則化の重み λ は、グリッド上での4分割交差検証によって選択し、平均の露出加重RMSE（Root Mean Square Error）を最小化する組合せを選んだ。結果について率直に報告すべき2つの特徴がある。第一に、交差検証の誤差曲面は k の広い範囲にわたってほぼ平坦であるため、選択されたランクは特定の内在的次元性の証拠としてではなく、低ランク交互作用項を最もよく正則化する値として読むべきである。第二に、この基準は低ランクかつ正則化の効いた解を選好する（選択値は $k=2$, $\lambda=10$）。これは、このデータセットにおいては交互作用項の主効果を超える寄与が限定的であるという第4.7節の知見と整合的である。
...
- 予測の信頼性：図4.5.1はホールドアウトセットにおける予測値と真値の正の相関を示しており、モデルが単に学習データに過適合しているわけではないことを示している。第4.7節の直接比較（head-to-head）が示すとおり、観測セルにおけるこのホールドアウト精度は主効果GLMを露出加重RMSEおよびポアソン逸脱度でわずかに上回るが、その差は小さい。行列分解の特徴的な価値はむしろ欠測セルの扱いにあり、これは同節および結論で論じる。
...
前節の行列分解は、過去の純保険料率の行列のみから潜在因子を学習する。しかし実務では、各車種グループ・各州について観測可能な属性（サイド情報）が利用できることが多い。集合行列分解（Collective Matrix Factorization, CMF; Cortes, 2018）は、主要な純保険料率行列を、行（車種グループ）および列（州）の属性行列と*同時に*分解することでこれらの属性を取り込む。属性は潜在因子を根拠づけ（ground）、観測の乏しい、あるいは未観測の車種グループ・州が、孤立した潜在因子に依存する代わりに、同じ属性を共有する仲間から強度を借りる（borrow strength）ことを可能にする。本節では前節の行列分解に対する拡張として、2種類のサイド情報を導入する。

- **車種側（行属性）：製造会社（メーカー）。** 車種グループのラベル先頭トークン（例：「Honda」「Gm」「Vw」）をメーカーとして抽出し、one-hot 符号化する。フィルタ後の231の車種グループに対し、51社のメーカーが存在する。

- **地域側（列属性）：州の人口密度区分。** 州の都市度（urbanicity）の代理指標であり、リスク特性と関連しうる。ブラジル地理統計院（IBGE）の2022年国勢調査による州別人口密度を用い、27州にわたって三分位で低・中・高の3区分（各9州）に分け、one-hot 符号化する（従来の連続対数密度を区分に置き換えた）。

　CMFは行列分解と同一のプロトコル（分割を先に行いその後チューニング、露出加重、非中心化、非負制約）で当てはめる。主行列・行属性・列属性の三つの損失に対する相対重み（$w_\mathrm{main}, w_\mathrm{user}, w_\mathrm{item}$）は、固定せず、主行列のホールドアウトRMSEを最小化するよう $(k, \lambda)$ と併せて交差検証で選択した。選択された重みは $(1.0,\ 0.05,\ 0.05)$、$(k, \lambda) = (22, 100)$ である。ただし交差検証のスコアはこれらの重みに対してほぼ平坦であり、サイド情報の重み付けが主行列の再現精度を左右するものではないことが示唆される。選択された重みが最小（サイド情報を最も軽く扱う）に寄っていること自体、このデータセットでは属性情報が主行列の再現に大きく寄与しないことを示唆している。予測性能の定量的な比較は次節（表4.5.1）で4モデルを同一のホールドアウトセル上で行う。図4.6.1にCMFの予測値対真値を示す。
...
- **本データセットではサイド情報は精度を改善しない：** 後掲の表4.5.1が示すとおり、CMFはすべての指標で素の行列分解を下回る（ポアソン逸脱度は $3.92\times10^7 \to 1.11\times10^8$、露出加重RMSEは $167.30 \to 253.73$ と、いずれも悪化）。衝突限定・全製造業者データにおいても、メーカーと州の人口密度区分は主行列（観測セル）の再現精度の向上には寄与しなかった。両露出層でも同様に悪化している。交差検証がサイド情報の重みを探索範囲の最小値 $(1.0,\ 0.05,\ 0.05)$ に寄せて選んだことも、これと整合的である。

- **通貨スケール指標は少数の高相対値セルに影響される：** 露出加重RMSEは相対値の大きい少数のセル（衝突純保険料率の最大値は約13,000）に影響を受けやすく、そこではサイド情報に基づく正則化がむしろ推定を歪める方向に働いた。この不利は重みのチューニングでは解消できず、構造的なものである。

- **素の行列分解にもGLMにも及ばない：** 観測セルの再現では素の行列分解が最も低い誤差を達成し、CMFは4モデルの中で最も劣る（表4.5.1）。CMFの価値は観測セルでの精度にあるのではなく、行列分解と同様、従来モデルが交互作用構造を提供できないスパース・欠測セグメントに対して、観測可能な属性に基づく推定を供給できる点にある。
...
表4.5.1：観測セルの同一547セット上でのホールドアウト予測性能（低いほど良い）。ハイパーパラメータは、最終フィットと同じ非負・非中心化（non-centered）・露出加重の仕様を用いた交差検証で再選択した（行列分解は $k=2$, $\lambda=10$、CMFは $k=22$, $\lambda=100$、サイド情報の重み $(1.0, 0.05, 0.05)$）。

| モデル | RMSE | 露出加重RMSE | ポアソン逸脱度 |
|---|---:|---:|---:|
| GLM（主効果） | 377.95 | 181.02 | 4.20×10⁷ |
| GLMM（変量交互作用）† | 377.95 | 181.02 | 4.20×10⁷ |
| 行列分解‡ | 373.87 | 167.30 | 3.92×10⁷ |
| 集合行列分解（CMF）‡ | 435.37 | 253.73 | 1.11×10⁸ |
...
‡ 行列分解の損失は（加重）二乗誤差であるのに対し、GLMとGLMMはポアソン尤度を最適化する。したがってMFは、露出加重RMSEとポアソン逸脱度の列において、それ自身が最小化していない基準で採点されており、これら2列では採点上の不利を負っている（それにもかかわらず、本結果ではMFが両列で最も低い誤差を達成している）。

露出加重を一貫して適用し、4モデルすべてを同一セル上で採点すると、行列分解が3指標すべてで最も低い誤差を達成する。ただしその差は小さく、露出加重RMSE（$167.30$ 対 $181.02$）およびポアソン逸脱度（$3.92\times10^7$ 対 $4.20\times10^7$）で主効果GLMをわずかに上回るにとどまる。GLMMは（上記の回帰の議論により）主効果GLMと同点となり、集合行列分解（CMF）はいずれの指標でも最も劣る。すなわち観測セルの再現において、低ランク交互作用の柔軟性は主効果ベースラインをわずかに上回る利得を生んだ一方、サイド情報の追加はむしろ精度を悪化させた。この結果は慎重に解釈する必要がある。この実験は*観測セルの再現*を測っており、そこでは主効果がすでに系統的変動の大半を説明するため、MFの追加的な交互作用の柔軟性が生む利得は小さい。これは、行列分解が本来意図する設定である*真に欠測したセルの推定*を測るものでは*ない*。それらのセルには真値が存在しないためである。

利用可能な唯一の真値—低露出の観測セル—を用いてスパース領域を精査するため、ホールドアウトセルを露出の中央値でさらに層別した（表4.5.2）。衝突事故の純保険料率を目的変数とした本分析では、行列分解がスパース層・密層のいずれでも最も低い露出加重RMSEを達成した（スパース 401.62、密 142.27）。主効果GLMはいずれの層でもこれに次ぎ（スパース 425.37、密 155.42）、集合行列分解（CMF）が最も劣った（スパース 467.80、密 235.38）。すなわち低ランク交互作用構造は、データの薄いスパース層でも主効果ベースラインをわずかに上回った。ただしポアソン逸脱度では順位が一様ではなく、密層ではMFが最小（2.62×10⁷）である一方、スパース層では主効果GLM（1.20×10⁷）がMF（1.31×10⁷）をわずかに下回る。いずれにせよ層別で見てもMFの優位は小さい。またこれはあくまで代理指標である点を強調しておく。真に欠測したセルはさらに少ない（典型的にはゼロの）露出しか持たず、これらの数値のいずれによっても保証できる範囲の外にある。

表4.5.2：露出で層別したホールドアウト性能（中央値で分割、スパース n=273・密 n=274、低いほど良い）。

| 層 | モデル | 露出加重RMSE | ポアソン逸脱度 |
|---|---|---:|---:|
| スパース（露出 < 中央値） | GLM / GLMM† | 425.37 | 1.20×10⁷ |
| スパース（露出 < 中央値） | 行列分解 | 401.62 | 1.31×10⁷ |
| スパース（露出 < 中央値） | 集合行列分解（CMF） | 467.80 | 1.93×10⁷ |
| 密（露出 ≥ 中央値） | GLM / GLMM† | 155.42 | 3.00×10⁷ |
| 密（露出 ≥ 中央値） | 行列分解 | 142.27 | 2.62×10⁷ |
| 密（露出 ≥ 中央値） | 集合行列分解（CMF） | 235.38 | 9.22×10⁷ |

したがってこの比較は、観測データ上で行列分解が主効果GLMをわずかに上回る一方、その差は小さく、精度そのものが行列分解の主たる利点ではないことを示している。その特徴的な貢献は精度の大小ではなく別のところ、まさに本稿が関心を寄せる領域にある。すなわち、GLMMが交互作用効果を全く決定できず、GLMが主効果を外挿することしかできない、露出がほとんど、あるいは全くないスパース・欠測セグメントである。そこでは、行列分解は3手法の中で唯一、交互作用を考慮した料率推定を依然として生成できる（第4.4節）。言い換えれば、その価値は表4.5.1の密に観測されたセルでのわずかな誤差低減にあるのではなく、*従来モデルが信頼できる交互作用構造を提供できないセグメントへ、それを拡張する*ことにある。採択済みアブストラクトの表現—MFが「GLMがしばしば失敗するスパースなセグメントについて効果的に料率を推定した」—は、この構造的な意味で読むべきである。すなわちMFの主たる意義は、GLMMが報告すべき変量効果を持たないセルに対して交互作用を考慮した推定を供給する点にあり、ここで報告した観測セル上でのGLMに対するわずかなホールドアウト誤差の優位そのものにあるのではない。

**限界：セル率に含まれる属性構成の交絡。** 本分析のセル目的変数（生の衝突純保険料率 = Σクレーム/Σ露出）は、車種グループ×州の交互作用に加えて、各セルの契約者属性の構成（性別・運転者年齢・車両年式のミックス）を混在させており、この構成はセル間で大きく異なる。頑健性確認として、レコード単位のポアソンGLMで性別・年齢・年式の相対度を推定し、車種グループと州を基準水準に固定した期待クレーム基準 $E^{*}$ を作って目的変数を標準化相対度 $r = \sum \text{クレーム} / \sum E^{*}$ に置き換え、属性ミックスを全モデル共通に除去して同一プロトコルで再比較した。その結果、観測セルでの順位は逆転し、主効果GLMがMFを全指標で上回った（露出加重RMSE 2,126 対 4,296、ポアソン逸脱度 $6.59\times10^{7}$ 対 $1.07\times10^{8}$；標準化相対度は生の純保険料とは異なるスケールであるため、比較は本確認内の相対順位として読むべきである）。これは、生の目的変数上でのMFのわずかな優位が、車種×地域の純粋な交互作用ではなく、潜在因子が属性構成の差を吸収したことに部分的に由来する可能性を示唆する。したがって表4.5.1の観測セル精度におけるMFの優位はこの交絡を念頭に慎重に解釈すべきであり、MFの本質的価値は依然として、従来モデルが交互作用構造を提供できないスパース・欠測セグメントへ交互作用を考慮した推定を拡張できる点にある。

## 露出しきい値に対する感度分析

セルの採否に用いた露出しきい値（総露出100以上）は信頼性を担保するための設計上の選択であり、その値自体はモデリング上の裁量である。そこで、しきい値を50・100・200に変えて同一プロトコル（分割を先に行いその後チューニング、露出加重、非負・非中心化、$(k,\lambda)$・CMF重みの再選択）で再実行し、結論の頑健性を確認した。観測セル数は閾値50/100/200でそれぞれ2,688/2,233/1,771（評価セル654/547/426）と変化する。

表4.7.1に全体の露出加重RMSEを示す。閾値50および100（本文設定）では行列分解が主効果GLMをわずかに下回り最良であったが、閾値200では順位が逆転し主効果GLMが最良となった。CMFはいずれの閾値でも最も劣った。すなわち観測セルの再現における行列分解のわずかな優位は、本文設定およびそれ以下の閾値では認められるものの普遍的ではなく、閾値を高くして密なセルに限定すると主効果GLMが上回る。この感度は、本文で述べた「観測セル上での行列分解の優位は小さく、精度そのものが行列分解の主たる利点ではない」という慎重な解釈と整合的である。

表4.7.1：露出しきい値を変えたときの全体ホールドアウト露出加重RMSE（低いほど良い）。各行は独立に再チューニングした。100が本文の設定である。

| 露出しきい値 | 観測セル | 評価セル | GLM | 行列分解 | CMF |
|---|---:|---:|---:|---:|---:|
| 50 | 2,688 | 654 | 185.6 | 171.8 | 206.3 |
| 100（本文） | 2,233 | 547 | 181.0 | 167.3 | 253.7 |
| 200 | 1,771 | 426 | 131.4 | 138.2 | 172.2 |

# 結論と今後の課題

本研究では、クラス別料率算定に焦点を当て、多数のカテゴリと著しいスパース性を特徴とするデータセットに対するアプローチとして行列分解（MF）を検討した。本手法を実世界の保険データに適用した結果、その特徴的な利点は予測精度の一様な向上ではなく構造的なものであることがわかった。すなわち、未観測の組合せに対して交互作用変量効果が主効果に回帰してしまうGLMMとは異なり、MFは観測露出のないものを含め、すべての車種グループ×州セルに対して交互作用を考慮した推定を生成する。観測セル上での対等な（like-for-like）ホールドアウト比較では—露出加重損失と比較可能な指標を用いて—行列分解が3指標すべてで最も低い誤差を達成したが、その差は主効果GLMをわずかに上回る程度であった。GLMMは、ホールドアウトの各セルが未観測交互作用でありその変量効果がゼロに回帰するためGLMと同点となった（表4.5.1）。層別分析でも、行列分解がスパース（低露出）層・密層のいずれでも最も低い露出加重RMSEを達成し、GLM・CMFはこれに次いだ（表4.5.2）。すなわち、衝突事故の純保険料率を目的変数とした観測セルの再現において、低ランク交互作用構造は主効果ベースラインをわずかに上回った一方、サイド情報（CMF）は精度を改善しなかった。それでもなお、採択済みアブストラクトの「GLMがしばしば失敗するスパースなセグメント」への言及は、精度の優劣ではなく次の構造的な意味で読むべきである。すなわちMFの価値は、GLMMが主効果のまま残す多数の組合せに対して交互作用を考慮した料率を供給することにあり、ここで報告した観測セル上でのGLMに対するわずかなホールドアウト誤差の優位そのものにあるのではない。したがって我々はMFを、確立されたアクチュアリーモデルの全面的な代替としてではなく、従来モデルが過学習するか主効果に帰着してしまうスパースで高次元の交互作用設定でこそ最大の価値を持つ補完的ツールとして位置づける。

　本研究は提案アプローチの有効性を確認したが、さらなる発展の余地がいくつか残されている：

- サイド情報の統合：本稿では第4.6節で、主要な事故行列をメーカーと州の人口密度区分の補助属性行列とともに同時に分解する集合行列分解（Collective Matrix Factorization, CMF）を検証した。衝突事故の純保険料率を目的変数とした本データセットでは、サイド情報（メーカー・州の人口密度区分）は観測セルの再現精度を向上させず、CMFはすべての指標で素の行列分解を下回った。交差検証がサイド情報の重みを探索範囲の最小値に寄せて選んだことも、これと整合的である。もっとも、これはサイド情報が原理的に無効であることを意味しない。今後の課題としては、より豊富な属性（車両の物理特性、地域の社会経済指標など）の取り込みや、サイド情報が真に欠測したセルの推定に寄与するかの直接的な検証が挙げられる。

- 純保険料に整合した損失族：提案モデルは（加重）二乗誤差損失を最小化するが、これは非負で裾の重い純保険料を近似的にガウス分布として扱い、最大のセルに支配される。GLMとGLMMがポアソン尤度を最適化する一方、MFは逸脱度ベースの指標を直接最適化しておらず、この目的関数の不整合はMFにとって構造的に不利に働きうる（それでもなおMFは表4.5.1の逸脱度ベース指標で最も低い値を達成した）。自然な拡張は、TweedieまたはポアソンガンマのDeviance損失（例：Jørgensen & Paes de Souza, 1994; Ohlsson & Johansson, 2010）の下で分解を当てはめ、目的関数を保険事故の頻度・severity構造に整合させることである。
