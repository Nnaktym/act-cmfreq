...
$$\mathrm{PP}_{ij} \sim \mathrm{Tweedie}(p=1.5),\qquad \ln E[\mathrm{PP}_{ij}] = \beta_0 + \alpha_i + \tau_j + z_{ij}$$

ここで zij~$N(0, \delta^2)$ は車種 i と州 j の交互作用に対する変量効果を表す。すなわち主効果GLM（第4.3節）と同一の Tweedie（複合ポアソン–ガンマ, $p=1.5$）・対数リンク・露出を prior weight とする定式化の上に、車種×州の交互作用を変量効果として加えたものである。式中の他の変数の定義は第4.3節で述べたものと同じである。
...
本節では、cmfrecライブラリ（Cortes, 2018。本稿で報告する分析はそのPython実装を用いる）を用いて提案する行列分解アプローチを適用する。このライブラリはRとPythonの両方で利用可能な行列分解の標準的なツールである。勾配ベースの手法や交互最小二乗法（ALS）を含む様々な最適化アルゴリズムを実装し、L1およびL2正則化の両方をサポートする。

モデルは3.2節で定式化したとおりである。本研究の目的はクラス料率算定への応用であるため、上記のモデルをレコメンダーシステムの文脈から料率算定の文脈へ**読み替えて**用いる。レコメンダーシステムでは行列 $X$ の行・列・各要素がそれぞれ「ユーザー」「アイテム」「評価値（選好）」を表すが、本研究ではこれらを次表のように対応させる。行に対応させるのは**個別の車種（VehModel）**である。
...
　第2節で述べたとおり、多数のカテゴリを持つリスク因子間の交互作用の推定は、特定のカテゴリ組合せにおける欠測データのために課題となる。しかし提案アプローチでは、交互作用は潜在因子行列の積を用いた低ランク行列によって表現されるため、欠測データを含む組合せに対しても、各因子の効果を考慮した適切な予測が期待できる。
...
潜在因子数 k（候補：2〜27の整数）と正則化の重み λ（候補：0.01, 0.1, 1, 10, 20, 30, 50, 100, 1000）は、これらの候補からなるグリッド上での4分割交差検証によって選択し、平均の露出加重RMSE（Root Mean Square Error）を最小化する組合せを選んだ。k の探索上限を27としたのは、ランクが行列の小さい方の次元（27州）で頭打ちになるためである。テストセットはこのチューニングの前に取り分けてあり、ハイパーパラメータ選択が評価セルを見ることはない。**本研究（車種×州, 盗難）では、この手続きは k=8、正則化の重み λ=1000が採用された。** 
...
- 予測の信頼性：図4.5.1はホールドアウトセットにおける予測値と真値の正の相関を示しており、モデルが単に学習データに過適合しているわけではないことを示している。次節の直接比較（head-to-head）が示すとおり、本研究では観測セルにおいて行列分解が非加重RMSE・露出加重RMSEの**両指標**で主効果GLMを上回った。

## 予測性能の比較

前節までは各モデルの推定を個別に提示した。これらを対等な土俵で比較するため、主効果GLM、GLMM、および行列分解を、観測セルの同一のホールドアウトセット（1,502セル）上で評価した。公正な比較のためには2つの調整が不可欠である。第一に、行列分解の損失を露出で加重し、GLMとGLMMが Tweedie の prior weight として適用する露出加重と整合させる。第二に、純保険料のRMSEに加えて、露出加重RMSEを報告し、比較可能で露出を考慮した基準でモデルを判断する。
...
‡ 行列分解の損失は（加重）二乗誤差であるのに対し、GLMとGLMMはいずれも Tweedie 尤度を最適化する。したがってMFは、露出加重RMSEの列において、それ自身が最小化していない基準で採点されているにもかかわらず、両指標でGLMを上回った。

　プールした結果は、**単一分割（seed 123）においては一貫して行列分解に有利**である。行列分解は非加重RMSE（281.4 対 GLMの319.4）と露出加重RMSE（191.9 対 218.7）のいずれでも主効果GLMを下回った。GLMMは（各ホールドアウトセルが未観測交互作用で変量効果がゼロに回帰するため）GLMと完全に同点となる。
