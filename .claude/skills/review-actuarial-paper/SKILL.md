...
**This repo's paper:** `paper/ICA2026_Matrix_Factorization_for_Class_Ratemaking.md` (source) and the `.pdf` (rendered equations — read the PDF when the markdown math looks mangled). Method code (live pipeline): `src/helper.py`, `src/brazil_data_analysis_R.py`, `src/glmm_pymc.py`. The original R scripts, the pre-refactor Python experiment, and the variant/parameter-study scripts are archived under `archive/` (reference only). Data: `data/`.
